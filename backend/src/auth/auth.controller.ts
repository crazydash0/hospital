...
export class AuthController {
  ...
  @Post('verify-email')
  @Public()
  verifyEmail(@Body() body: { email: string; code: string }) { return this.authService.verifyEmail(body.email, body.code); }

  @Post('resend-email-verification')
  @Public()
  resendEmailVerification(@Body() body: { email: string }) { return this.authService.resendEmailVerification(body.email); }

  @Post('phone/request-code')
  @Public()
  requestPhoneCode(@Body() body: { phone: string }) { return this.authService.requestPhoneVerification(body.phone); }

  @Post('phone/verify-code')
  @Public()
  verifyPhoneCode(@Body() body: { phone: string; code: string; fullName?: string }) { return this.authService.verifyPhone(body.phone, body.code, body.fullName); }
  ...
  @Get('google/callback')
  @Public()
  async googleCallback(@Query('code') code: string, @Query('state') state: string) { return this.oauthCallback(AuthProvider.GOOGLE, code, state); }
  ...
  @Public()
  async facebookCallback(@Query('code') code: string, @Query('state') state: string) { return this.oauthCallback(AuthProvider.FACEBOOK, code, state); }
  ...
  private async oauthCallback(provider: AuthProvider, code: string, state: string) {
    ...
    if (provider === AuthProvider.GOOGLE) {
      ...
      return this.authService.loginWithOAuth(provider, profile.sub, profile.email, profile.name ?? 'Patient');
    }
    ...
    return this.authService.loginWithOAuth(provider, profile.id, profile.email ?? null, profile.name ?? 'Patient');
  }
}
