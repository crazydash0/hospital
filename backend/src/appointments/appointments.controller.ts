...
export class AppointmentsController {
  ...
  book(@Req() req, @Body() body: BookAppointmentDto) {
    return this.service.bookAppointment(req.user.userId, body.slotId, req.user);
  }

  @Roles(Role.DOCTOR)
  @Get('doctor')
  doctorAppointments(@Req() req) {
    return this.service.getDoctorAppointments(req.user.userId, req.user);
  }

  @Roles(Role.PATIENT)
  @Get('patient')
  patientAppointments(@Req() req) {
    return this.service.getPatientAppointments(req.user.userId, req.user);
  }
  ...
  setMeetingLink(@Param('id', ParseIntPipe) id: number, @Req() req, @Body() body: SetMeetingLinkDto) {
  ...
  getDoctorAvailableSlots(@Param('doctorId', ParseIntPipe) doctorId: number, @Req() req) {
    return this.service.getAvailableSlots(doctorId, req.user);
  }

  @Roles(Role.DOCTOR)
  @Post('slots')
  createSlots(@Req() req, @Body() body: CreateSlotsDto) {
    return this.service.createSlots(req.user.userId, body.date, body.startHour, body.endHour, body.duration, req.user);
  }

  @Roles(Role.DOCTOR)
  @Post('weekly-template')
  setWeeklyTemplate(@Req() req, @Body() body: SetWeeklyTemplateDto) {
    return this.service.setWeeklyTemplate(req.user.userId, body.dayOfWeek, body.startHour, body.endHour, body.duration, body.note, req.user);
  }

  @Roles(Role.DOCTOR)
  @Post('weekly-template/range')
  setWeeklyRange(@Req() req, @Body() body: SetWeeklyRangeDto) {
    return this.service.setWeeklyRange(req.user.userId, body.fromDay, body.toDay, body.startHour, body.endHour, body.duration, body.note, req.user);
  }

  @Roles(Role.DOCTOR)
  @Delete('weekly-template/:dayOfWeek')
  deleteWeeklyTemplateDay(@Req() req, @Param('dayOfWeek', ParseIntPipe) dayOfWeek: number) {
    return this.service.deleteWeeklyTemplateDay(req.user.userId, dayOfWeek, req.user);
  }

  @Roles(Role.DOCTOR)
  @Get('weekly-template')
  getWeeklyTemplate(@Req() req) {
    return this.service.getWeeklyTemplate(req.user.userId, req.user);
  }

  @Get('weekly-template/doctor/:doctorId')
  getPublicWeeklyTemplate(@Param('doctorId', ParseIntPipe) doctorId: number, @Req() req) {
    return this.service.getPublicWeeklyTemplate(doctorId, req.user);
  }

  @Roles(Role.DOCTOR)
  @Post('leaves')
  addLeave(@Req() req, @Body() body: CreateLeaveDto) {
    return this.service.addLeave(req.user.userId, body.date, req.user);
  }

  @Roles(Role.DOCTOR)
  @Get('leaves')
  getLeaves(@Req() req) {
    return this.service.getDoctorLeaves(req.user.userId, req.user);
  }

  @Roles(Role.DOCTOR)
  @Delete('leaves/:id')
  removeLeave(@Req() req, @Param('id', ParseIntPipe) id: number) {
    // Leave deletion is date-based in the clinic-aware service; keep the legacy endpoint
    // contract by loading the leave first through the clinic-scoped doctor list.
    return this.service.removeLeaveById(req.user.userId, id, req.user);
  }
}